...
artifacts_directory: Path = here.joinpath("artifacts").resolve()
checkpoints: Path = here.joinpath("checkpoints").resolve()
data_directory: Path = resources.joinpath("data").resolve()
...
class Model(ReprMixin, metaclass=ABCMeta):
    ...
    @property
    @abstractmethod
    def schema_fields(self: Model) -> dict:
...
@unique
class ConfusionMatrixColumns(Enum):
...
class ConfusionMatrix:
    ...
    def precision(self: ConfusionMatrix) -> MetricScore:
        ...
        References:
            - https://developers.google.com/machine-learning/crash-course/classification/precision-and-recall#precision
    ...
    @cached_property
    def error(self: ConfusionMatrix) -> MetricScore:
...
def get_truth_statistics(
    ...
    return DataFrame(
        {
            ConfusionMatrixColumns.TRUE_POSITIVE.value: Series(
                [true_positives], dtype=np.int64
            ),
            ConfusionMatrixColumns.FALSE_POSITIVE.value: Series(
                [false_positives], dtype=np.int64
            ),
            ConfusionMatrixColumns.TRUE_NEGATIVE.value: Series(
                [true_negatives], dtype=np.int64
            ),
            ConfusionMatrixColumns.FALSE_NEGATIVE.value: Series(
                [false_negatives], dtype=np.int64
            ),
            ConfusionMatrixColumns.ACCURACY.value: Series([accuracy], dtype=np.float64),
            ConfusionMatrixColumns.TRUE_POSITIVE_RATE.value: Series(
                [true_positive_rate], dtype=np.float64
            ),
            ConfusionMatrixColumns.TRUE_NEGATIVE_RATE.value: Series(
...
def plot_correlation_matrix(df: DataFrame, title: str, output_dir: Path) -> DataFrame:
