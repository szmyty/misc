...
def long_short_portfolio(
    ...
    portvar: str,
    byvars: Optional[Union[str, List[str]]] = None,
    retvars: Optional[Union[str, List[str]]] = None,
    top_minus_bot: bool = True,
):
    """
...
def _portfolio_difference(df, portvar, long, short, byvars=None, retvars=None):
    ...
    """
    if byvars:
        out = df[df[portvar] == long].set_index(byvars) - df[
            df[portvar] == short
        ].set_index(byvars)
    else:
...
def _portfolio_match(elem, port_cutoffs):
    if np.isnan(elem) or np.isinf(elem):
        return 0
    return [
        index + 1 for index, bot, top in port_cutoffs if elem >= bot and elem <= top
    ][0]
...
def _sort_arr_list_into_ports(
    ...
    common_args = (array_list, cut_array_list, percentiles)
    common_kwargs = dict(cutoffs=cutoffs)
    if multiprocess:
        if isinstance(multiprocess, int):
            return _sort_arr_list_into_ports_mp(
                *common_args, mp=multiprocess, **common_kwargs
            )
        else:
...
def _create_cutoffs_arr_if_necessary_and_sort_into_ports(
    data_tup, percentiles, cutoffs: Optional[List[Union[float, int]]] = None
):
    arr, cutarr = data_tup
    if cutoffs is None:
...
def _sort_arr_list_into_ports_sp(
    array_list,
    cut_array_list,
    percentiles,
    cutoffs: Optional[List[Union[float, int]]] = None,
    ...
    for i, arr in enumerate(array_list):
        result = _create_cutoffs_arr_if_necessary_and_sort_into_ports(
            (arr, cut_array_list[i]), percentiles, cutoffs=cutoffs
        )
...
def _sort_arr_list_into_ports_mp(array_list, cut_array_list, percentiles, mp=None):
    if mp:
        with Pool(mp) as pool:  # use mp # processors
            return _sort_arr_list_into_ports_mp_main(
                array_list, cut_array_list, percentiles, pool
            )
    else:
...
def _sort_arr_list_into_ports_mp_main(
    array_list,
    cut_array_list,
    percentiles,
    pool,
    ...
    # Mp setup
    port = partial(
        _create_cutoffs_arr_if_necessary_and_sort_into_ports,
        percentiles=percentiles,
        cutoffs=cutoffs,
    )
    ...
    results = [
        pool.apply_async(port, ((arr, cut_array_list[i]),), callback=counter.append)
        for i, arr in enumerate(array_list)
    ]
...
def _sort_arr_list_into_ports_and_return_series(
    array_list,
    cut_array_list,
    percentiles,
    multiprocess,
    ...
    al = _sort_arr_list_into_ports(
        array_list, cut_array_list, percentiles, multiprocess, cutoffs=cutoffs
    )
...
def _check_portfolio_inputs(*args, **kwargs):
    ...
    if user_passed_any_cutoffs and kwargs["ngroups"] not in [
        10,
        None,
        0,
    ]:  # 10 is default ngroups
        raise ValueError(
            f'cannot pass both cutoffs and ngroups. got {kwargs["cutoffs"]} cutoffs '
            f'and {kwargs["ngroups"]} ngroups'
        )

    if user_passed_any_cutoffs and kwargs["cutdf"] is not None:
        warnings.warn(
            f'cutdf will not be used for portfolios as cutoffs {kwargs["cutoffs"]} were passed'
        )

    if user_passed("cutoffs") and user_passed("quant_cutoffs"):
        raise ValueError(
            f'cannot pass both cutoffs and quant_cutoffs. got {kwargs["cutoffs"]} cutoffs and '
