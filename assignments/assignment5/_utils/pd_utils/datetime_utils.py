...
def convert_sas_date_to_pandas_date(
    sasdates: Union[pd.Series, int]
...
def year_month_from_date(
    ...
    df[[yearname, monthname]] = apply_func_to_unique_and_merge(
        df[date], year_month_from_single_date
    )
...
def expand_months(
    df: pd.DataFrame,
    datevar: str = "Date",
    newdatevar: str = "Daily Date",
    trade_days: bool = True,
):
    """
...
def _expand(monthly_date, datevar, td, newdatevar):
    t = time.gmtime(
        monthly_date / 1000000000
    )  # date coming in as integer, need to parse
