    ...
    **N.B.** The encoding works on a **per-character** basis. Thus, all keys in the
    ...
    if not same_length:
        sequences = pad_trim_sequences(
            sequences, maxlen=maxlen, align=align, pad_value=pad_value
        )
    ...
    all_encoded_sequences = [
        encode_sequence(sequence, encoding_map, flatten=flatten) for sequence in it
    ]
...
def encode_sequence(
    sequence: str, encoding_map: encoding_map_type, flatten: bool = False
) -> np.ndarray:
    ...
    try:
        encoded_sequence = np.array([encoding_map.get(a) for a in sequence])
...
def pad_sequence(
    seq: str, max_seq_len: int, pad_value: str = "J", align: str = "end"
) -> str:
...
def pad_trim_sequences(
    ...
    if isinstance(seq_vec, str):
        seq_vec = [seq_vec]
    assert (
        isinstance(seq_vec, list)
        or isinstance(seq_vec, pd.Series)
        or isinstance(seq_vec, np.ndarray)
    )
    ...
    if max_seq_len < maxlen:
        msg = "Maximum sequence length (%s) is less than maxlen (%s)" % (
            max_seq_len,
            maxlen,
        )
        logger.warning(msg)
    ...
    # pad and trim
    padded_seq_vec = [
        pad_sequence(seq, max(max_seq_len, maxlen), pad_value=pad_value, align=align)
        for seq in seq_vec
    ]

    padded_seq_vec = [trim_sequence(seq, maxlen, align=align) for seq in padded_seq_vec]
...
def trim_sequence(seq: str, maxlen: int, align: str = "end") -> str:
