...
def is_iterator_exhausted(
    iterator: Iterable, return_element: bool = False
...
def list_to_dict(l: Sequence, f: Optional[Callable] = None) -> Dict:
    ...
    if len(l) % 2 != 0:
        msg = (
            "[collection_utils.list_to_dict]: the list must contain an even number"
            "of elements"
        )
...
def get_set_pairwise_intersections(
    dict_of_sets: Mapping[str, Set], return_intersections: bool = True
) -> pd.DataFrame:
