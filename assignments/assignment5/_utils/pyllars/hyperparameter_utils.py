        ...
        sklearn.model_selection.ParameterGrid(
            {
                ...
                "{}C".format(estimator_prefix): np.concatenate(
                    [10.0 ** -np.arange(0, 2), 10.0 ** np.arange(1, 3)]
                ),
                "{}gamma".format(estimator_prefix): [
                    1,
                    0.1,
                    0.01,
                    0.001,
...
def _get_res(res):
    ret_val = {"validation_{}".format(k): v for k, v in res.metrics_val.items()}

    ret_test = {"test_{}".format(k): v for k, v in res.metrics_test.items()}
...
def get_hp_results(all_res):
    ...
    results = [_get_res(res) for res in all_res]
...
def get_best_hyperparameters(
    results, evaluation_metric, ex_type="max", group_fields="fold_val"
):
