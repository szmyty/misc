...
def apply(
    df: pd.DataFrame, func: Callable, *args, progress_bar: bool = False, **kwargs
) -> List:
    ...
    ret_list = [func(*(row[1], *args), **kwargs) for row in it]
...
def apply_groups(
    ...
    **kwargs,
    ...
    ret_list = [func(*(group, *args), **kwargs) for name, group in it]

    return ret_list


def dict_to_dataframe(
    dic: Dict, key_name: str = "key", value_name: str = "value"
) -> pd.DataFrame:
...
def get_series_union(*pd_series: Iterable[pd.Series]) -> Set:
    ...
    """
    res = set.union(*[set(s) for s in pd_series])
...
def read_df(
    filename: str, filetype: str = "AUTO", sheet: str = None, **kwargs
) -> pd.DataFrame:
...
def write_df(
    ...
    **N.B.** The hdf5 filetype has not been thoroughly tested.
    ...
    if "do_not_compress" in kwargs:
        msg = (
            "[pd_utils.write_df] `do_not_compress` keyword has been removed. "
    ...
    if create_path:
        if filetype != "excel_writer":
            ...
            msg = (
                "[pd_utils.write_df]: create_path was passed as True, but the "
...
def split_df(
    df: pd.DataFrame, num_groups: int = None, chunk_size: int = None
    ...
    if num_groups is None:
        if chunk_size is not None:
            ...
            msg = (
                "[pd_utils.split_df] one of `num_groups` and `chunk_size` "
                "must be provided"
            )
            raise ValueError(msg)
...
def group_and_chunk_df(
    df: pd.DataFrame, groupby_field: str, chunk_size: int
) -> pd.core.groupby.DataFrameGroupBy:
...
def get_group_sizes(
    ...
    groups: Optional[pd.core.groupby.GroupBy] = None,
) -> pd.DataFrame:
    ...
    if (group_fields is None) and (groups is None):
        msg = "[pandas_utils.get_group_sizes]: No groups or group field " "provided"
        raise ValueError(msg)

    # we also can't have both
    if (group_fields is not None) and (groups is not None):
        msg = "[pandas_utils.get_group_sizes]: Both groups and group field " "provided"
        raise ValueError(msg)
...
def join_df_list(
    dfs: List[pd.DataFrame], join_col: StrOrList, *args, **kwargs
) -> pd.DataFrame:
...
def cross_product(
    df_left: pd.DataFrame, df_right: pd.DataFrame, tmp_key: str = "_tmpkey", **kwargs
) -> pd.DataFrame:
...
def filter_rows(
    ...
    *must* match.
    ...
    d = df_filter.merge(
        df_to_keep, left_on=filter_on, right_on=to_keep_on, indicator=True, how="outer"
    )
