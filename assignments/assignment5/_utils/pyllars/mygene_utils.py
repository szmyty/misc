...
def get_entrez_gene_mapping(ensembl_protein_ids: Iterable[str]) -> Dict:
    ...
    mapping = {record["query"]: record.get("entrezgene") for record in records}

    return mapping


def parse_go_terms(
    row: pd.Series, go_hierarchies: Iterable[str] = ["BP", "MF", "CC"]
) -> Dict:
...
def query_mygene(
    ...
    mygene_url: str = "http://mygene.info/v3",
) -> pd.DataFrame:
    ...
    res = MG.getgenes(
        gene_ids, fields=FIELDS, as_dataframe=True, species=species, scopes=scopes
    )
    res = res.reset_index()
