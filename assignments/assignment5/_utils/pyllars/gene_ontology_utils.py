...
go_sub_ontology_terms = collections.namedtuple(
    "go_sub_ontology_terms", ",".join(go_sub_ontology_terms)
)
...
_DEPRECATED_SUBONTOLOGY_TERM_SETS = {
    "molecular_function": {
        k
        for k, v in _DEPRECATED_TERM_SUBONTOLOGIES.items()
        if v == "molecular_function"
    },
    "biological_process": {
        k
        for k, v in _DEPRECATED_TERM_SUBONTOLOGIES.items()
        if v == "biological_process"
    },
    "cellular_component": {
        k
        for k, v in _DEPRECATED_TERM_SUBONTOLOGIES.items()
        if v == "cellular_component"
    },
}
...
def get_ancestor_terms(go_term, go_dag, include_go_term=True):
    ...
    if paths is not None:
        all_ancestors = {term.id for path in paths for term in path}
...
def get_sub_ontology_terms(go_dag):
    ...
    """
    biological_process_terms = get_descendent_terms(
        BIOLOGICAL_PROCESS_ROOT_TERM, go_dag
    )
    cellular_component_terms = get_descendent_terms(
        CELLULAR_COMPONENT_ROOT_TERM, go_dag
    )
    molecular_function_terms = get_descendent_terms(
        MOLECULAR_FUNCTION_ROOT_TERM, go_dag
    )
...
def get_gene_go_annotations(gene, gene2go, include_ancestors=False, go_dag=None):
    ...
    if include_ancestors:
        ...
        annotations = [(gene, go_term) for go_term in gene2go[gene]]
    return annotations
