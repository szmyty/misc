...
def write_sparse_matrix(
    target: str, a: scipy.sparse.spmatrix, compress: bool = True, **kwargs
) -> None:
...
# Other helpers
###
def matrix_multiply(m1: np.ndarray, m2: np.ndarray, m3: np.ndarray) -> np.ndarray:
...
def permute_matrix(
    m: np.ndarray, is_flat: bool = False, shape: Optional[Tuple[int]] = None
) -> np.ndarray:
