...
def get_mcar_incomplete_data(X, missing_likelihood=0.1, random_state=8675309):
    ...
    X_incomplete, missing_mask = math_utils.mask_random_values(
        X, likelihood=missing_likelihood, return_mask=True, random_state=random_state
    )
...
def get_nmar_incomplete_data(X, missing_likelihood=None, random_state=8675309):
    ...
    if len(missing_likelihood) != X.shape[1]:
        msg = (
            "[get_nmar_complete_data]: the number of functions for "
...
def remove_y_when_z_is_large(x, y, z, threshold, combination_operator=np.product):
...
def get_incomplete_data(X, mechanism, missing_likelihood, random_state=8675309):
    ...
    if mechanism == "mcar":
        ...
        msg = (
            "[get_incomplete_data]: unknown missing data mechansim: {}. Must "
...
IncompleteDataset = collections.namedtuple(
    "IncompleteDataset", _incomplete_dataset_fields
)
...
def add_identities(X, node_identities=None):
    ...
    if scipy.sparse.issparse(X):
        ...
        if X.ndim != 2:
            msg = (
                "[add_node_identities]: attempting to add identities to "
                "an np.array with invalid number of dimensions: {}".format(X.ndim)
            )
            raise ValueError(msg)
        ...
        msg = (
            "[add_node_identities]: attempting to add identities to an "
...
TrainingResults = collections.namedtuple("TrainingResults", _training_results_fields)
...
def replace_nans_with_zero(X, y=None, try_cast=True):
    if try_cast:
        try:
            ...
            msg = (
                "[replace_nans_with_zero] the input data type could not "
...
def get_nan_preprocessing_pipeline(
    dataset_manager, fill_categoricals="flag", fields_to_ignore=None
):
    ...
    if fill_categoricals not in allowed_fill_categoricals:
        msg = (
            "[missing_data_utils.get_nan_preprocessing_pipeline] allowed "
    ...
    num_imputer_template = FunctionTransformer(
        func=replace_nans_with_zero, validate=False
    )
    ...
    if hasattr(dataset_manager, "le_"):
        if fill_categoricals == "mode":
            ...
            cat_imputer_template = CategoricalImputer(missing_values="NaN")
        elif fill_categoricals == "flag":
            cat_imputer_template = FunctionTransformer(
                func=replace_nans_with_flag, validate=False
            )
        ...
        one_hot_encoder = NaNOneHotEncoder(
            categorical_features=dataset_manager.get_categorical_field_indices(
                fields_to_ignore
            ),
            sparse=False,
            n_values=n_values
        ...
        msg = (
            "[missing_data_utils.get_nan_preprocessing_pipeline]: did not "
