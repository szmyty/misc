...
def validate_1d(array, name="array", caller=None):
    ...
    if len(array.shape) != 1:
        _raise_value_error("{caller}{name} must be a 1-d data structure", name, caller)
...
def validate_2d(array, name="array", caller=None):
    ...
    if len(array.shape) != 2:
        _raise_value_error("{caller}{name} must be a 2-d data structure", name, caller)
...
def validate_all_between(array, min_val, max_val, name="array", caller=None):
    ...
    valid_vals = [
        check_range(v, min_val, max_val, raise_on_invalid=False) for v in array
    ]
...
def validate_all_finite(array, name="array", caller=None):
    ...
    if not np.isfinite(array).all():
        _raise_value_error(
            "{caller}{name} is not allowed to include " "non-finite values",
            name,
            caller,
        )
...
def validate_all_non_negative(array, name="array", caller=None):
    ...
    if scipy.sparse.issparse(array):
        ...
        if min_val < 0:
            _raise_value_error(
                "{caller}{name} is not allowed to include " "negative values",
                name,
                caller,
            )
    else:
...
def validate_all_positive(array, name="array", caller=None):
    ...
    if np.any(array <= 0):
        _raise_value_error(
            "{caller}{name} is not allowed to include " "non-positive values",
            name,
            caller,
        )


def validate_equal_length(array1, array2, name1="array1", name2="array2", caller=None):
    ...
    if array1.shape[0] != array2.shape[0]:
        ...
        _raise_value_error("{caller}{name} must have the same length", name, caller)


def validate_equal_shape(array1, array2, name1="array1", name2="array2", caller=None):
    ...
    if array1.shape != array2.shape:
        # a bit of a hack to make the error message look correct
        name = "{},{}".format(name1, name2)
        _raise_value_error("{caller}{name} must have the same shape", name, caller)
...
def validate_integral(array, name="array", caller=None, array_int=None):
    ...
    if array_int is None:
        try:
            ...
            # definitely fail.
            _raise_value_error(
                "{caller}{name} is not allowed to include " "non-integral values",
                name,
                caller,
            )
...
def validate_in_set(value, valid_values, name="value", caller=None):
    ...
    if value not in valid_values:
        ...
        msg = (
            "{caller}{name}. the given value is not valid. found: '"
            + value
            + "'. allowed: "
            + str(vv)
        )
...
def validate_is_sequence_of_integers(sequence, name="sequence", caller=None):
    ...
    if isinstance(sequence, np.ndarray) or scipy.sparse.issparse(sequence):
        ...
        msg = (
            "unrecognized data type for (sequence of) integers: value: {}. "
...
def validate_numeric(array, name="array", caller=None):
    ...
    if not np.issubdtype(array.dtype, np.number):
        msg = (
            "{caller}{name} invalid dtype for numeric sequences. found: " + array.dtype
        )
        _raise_value_error(msg, name, caller)


def validate_packages_installed(
    packages: typing.Iterable[str], caller: str = None
) -> None:
    ...
    missing = [(importlib.util.find_spec(p) is None) for p in packages]
...
def validate_type(var, allowed_types, name="var", caller=None):
    ...
    if not is_valid:
        ...
        msg = (
            "{caller}{name} invalid type. found: " + str(type(var)) + ". allowed: " + at
        )
...
def validate_unique(array, name="array", caller=None):
    ...
    if len(array) != len(set(array)):
        _raise_value_error(
            "{caller}{name} is not allowed to include " "duplicate values", name, caller
        )
...
###
def check_is_fitted(
    estimator, attributes, name="estimator", caller=None, all_or_any=all
):
    ...
    if not all_or_any([hasattr(estimator, attr) for attr in attributes]):
        msg = (
            "{caller}{name} is not fitted yet. Call the appropriate "
...
def validate_is_sequence(
    ...
    if (not validated) and raise_on_invalid:
        msg = (
            "{caller}{name} invalid type. found: "
            + str(type(maybe_sequence))
