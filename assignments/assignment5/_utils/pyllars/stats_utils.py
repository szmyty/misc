...
def calculate_symmetric_kl_divergence(
    p: Any, q: Any, calculate_kl_divergence: Callable
) -> float:
...
def calculate_univariate_gaussian_kl(
    mean_p_var_p: Tuple[float, float], mean_q_var_q: Tuple[float, float]
) -> float:
...
def get_population_statistics(
    ...
    subpopulation_variances: np.ndarray,
    ...
    _subpopulation_variance = np.multiply(
        subpopulation_sizes - 1, subpopulation_variances
    )
    ...
    _subpopulation_means_product = np.multiply(
        subpopulation_sizes, _subpopulation_mean_difference
    )
    _subpopulation_means_sum = np.sum(_subpopulation_means_product)

    population_variance = (_subpopulation_means_sum + _subpopulation_variance_sum) / (
        n - 1
    )
...
def sample_dirichlet_multinomial(
    dirichlet_alphas: np.ndarray, num_samples: int
) -> np.ndarray:
...
def sample_beta_binomial(
    alpha: float, beta: float, n_samples: int = 1, size: numpy_size = None
) -> sampling_return_type:
...
def sample_gamma_poisson(
    mean: float, var: float, size: numpy_size = None
) -> sampling_return_type:
