...
def distribute(cmd, node=None, node_list=None, connection_timeout=5, max_tries=10):
...
def write_results_to_file(
    out_filename, commands, node_list, return_codes, stdouts, stderrs
):
...
def add_ssh_options(
    ...
    ssh_options.add_argument(
        "--node",
        help="The node to which ssh will distribute " "jobs by default",
        default=default_node,
    )

    ssh_options.add_argument(
        "--node-list",
        help="A file containing a list of node "
        "names (one on each line, and only the node name) to which jobs will be "
        "randomly distributed",
        default=default_node_list,
    )

    ssh_options.add_argument(
        "--connection-timeout",
        help="The number of seconds to " "wait to establish a connection",
        type=int,
    ...
    ssh_options.add_argument(
        "--max-tries",
        help="The number of times to try to " "establish a connection",
        type=int,
