...
class NaNLabelEncoder(sklearn.base.BaseEstimator, sklearn.base.TransformerMixin):
    ...
    def fit(self, y):
        ...
        if pd.isnull(self.missing_value_marker):
            msg = (
                "[nan_label_encoder]: cannot use pd.isnull objects as the "
        ...
        if self.missing_value_marker in y:
            msg = "[nan_label_encoder]: found the missing value marker in " "the array"
            raise ValueError(msg)
        ...
        self.classes_ = {c: i for i, c in enumerate(self.classes_)}
    ...
    def transform(self, y):
        ...
        if len(self.classes_) < 20:
            msg = (
                "[nan_label_encoder.transform] observed labels: {}. "
                "self.classes_: {}".format(observed_labels, self.classes_)
            )
            logger.debug(msg)
        else:
            msg = "[nan_le.transform] too many classes to print observed " "labels"
            logger.debug(msg)

        unknown_labels = [l for l in observed_labels if l not in self.classes_]
        if len(unknown_labels) > 0:
            msg = "[nan_label_encoder.transform] y contains new labels: {}".format(
                str(unknown_labels)
            )
            raise ValueError(msg)
