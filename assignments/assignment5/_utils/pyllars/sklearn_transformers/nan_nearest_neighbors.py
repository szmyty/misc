...
class NanNearestNeighbors(object):
    ...
    def fit(self, X, *_, **__):
        ...
        distance_matrix = scipy.spatial.distance.pdist(X, metric=self.knn_metric_)
    ...
    def kneighbors(self, X, n_neighbors=None, return_distance=False, as_np=False):
        ...
        distance_matrix = scipy.spatial.distance.cdist(
            queries, indexed, metric=self.knn_metric_
        )
        ...
        ret = self.knn_.kneighbors(
            X=distance_matrix, n_neighbors=n_neighbors, return_distance=return_distance
        )
        ...
        if not as_np:
            ret_indices = [list(ret_indices[i]) for i in range(ret_indices.shape[0])]
