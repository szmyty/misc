...
class MultiColumnImputer(sklearn.base.TransformerMixin):
    ...
    def __init__(self, imputer_template, columns=None):
    ...
    def fit(self, X, *_, **__):
        ...
        if isinstance(X, np.ndarray):
            ...
            msg = (
                "[multicolumn_ci]: attepting to impute an object which is "
                "neither an np.array nor a pd.DataFrame. type: {}".format(type(X))
            )
