...
def connect(
    ...
    dask.distributed.Client, typing.Optional[dask.distributed.LocalCluster]
]:
...
def add_dask_options(
    ...
    dask_options.add_argument(
        "--cluster-location",
        help="The address for the "
    ...
    dask_options.add_argument(
        "--num-procs",
        help="The number of processes to use " "for a local cluster",
        type=int,
        default=num_procs,
    )
    ...
    dask_options.add_argument(
        ...
        "args.num_threads_per_cpu).",
        type=int,
        default=num_threads_per_proc,
    )
...
def get_dask_cmd_options(args: argparse.Namespace) -> List[str]:
    ...
    dask_options = ["num_procs", "num_threads_per_proc", "cluster_location"]
...
def apply_df(
    ...
    **kwargs,
    ...
    ret_list = [
        client.submit(func, *(row[1], *args), **kwargs, priority=priority) for row in it
    ]
...
def cancel_all(
    f_list: Iterable[dask.distributed.client.Future], pending_only=True
) -> None:
