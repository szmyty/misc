...
def fix_mimic_icds(icds: Iterable[str]) -> List[str]:
    ...
    """
    fixed_icds = [_fix_mimic_icd(icd) for icd in icds]
...
def get_diagnosis_icds(
    mimic_base: str, drop_incomplete_records: bool = False, fix_icds: bool = False
) -> pd.DataFrame:
...
def get_lab_events(
    mimic_base, to_pandas=True, drop_missing_admission=False, parse_dates=True, **kwargs
):
    ...
    if drop_missing_admission:
        msg = (
            "[physionet.get_lab_events] removing lab events with no "
...
def create_followups_table(mimic_base, progress_bar=True):
    ...
    all_followup_dfs = pd_utils.apply_groups(
        g_admissions, _get_followups, progress_bar=progress_bar
    )
...
CinC_2012_BOOKKEEPING_FIELDS = {"HADM_ID"}
...
CinC_2012_DESCRIPTOR_FIELDS = {"AGE", "GENDER", "HEIGHT", "ICU_TYPE", "WEIGHT"}
...
def _get_cinc_2012_record_descriptor(record_file_df):
    ...
    record_descriptor = pd_utils.dataframe_to_dict(
        record_descriptor, "Parameter", "Value"
    )
...
def get_cinc_2012_record(cinc_2012_base, record_id, wide=True):
    ...
    new_columns = {"Time": "ELAPSED_TIME", "Parameter": "MEASUREMENT", "Value": "VALUE"}
