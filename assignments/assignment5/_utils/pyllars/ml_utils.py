...
def get_train_val_test_splits(
    ...
    ret = split_masks(m_train, m_validation, m_test)
...
def get_fold_data(
    ...
    **N.B.** This function creates copies of the data, so it is not appropriate
    ...
    validation_utils.validate_is_sequence(
        attribute_fields, name="attribute_fields", caller=caller
    )

    if len(attribute_fields) == 1:
        msg = (
            "[{}]: found a single attribute field. Treating as a "
...
def _train_and_evaluate(
    ...
    # evaluate
    metrics = collect_metrics(y_test, y_pred, **collect_metrics_kwargs)
    ...
    **N.B.** This function is not particularly efficient with
    ...
    if train_new_test_model:
        ...
        metrics_test = collect_metrics(
            y_test, val_fold_data.y_test, **collect_metrics_kwargs
        )
...
def collect_regression_metrics(
    ...
    ret = {
        "{}explained_variance".format(prefix): sklearn.metrics.explained_variance_score(
            y_true, y_pred
        ),
        "{}mean_absolute_error".format(prefix): sklearn.metrics.mean_absolute_error(
            y_true, y_pred
        ),
        "{}mean_squared_error".format(prefix): sklearn.metrics.mean_squared_error(
            y_true, y_pred
        ),
        # "{}mean_squared_log_error".format(prefix): sklearn.metrics.mean_squared_log_error(y_true, y_pred),
        "{}median_absolute_error".format(prefix): sklearn.metrics.median_absolute_error(
            y_true, y_pred
        ),
...
def collect_multiclass_classification_metrics(
    ...
    ret = {
        "{}cohen_kappa".format(prefix): sklearn.metrics.cohen_kappa_score(
            y_true, y_pred
        ),
        ...
        "{}micro_f1_score".format(prefix): sklearn.metrics.f1_score(
            y_true, y_pred, average="micro"
        ),
        "{}macro_f1_score".format(prefix): sklearn.metrics.f1_score(
            y_true, y_pred, average="macro"
        ),
        "{}hamming_loss".format(prefix): sklearn.metrics.hamming_loss(y_true, y_pred),
        "{}micro_precision".format(prefix): sklearn.metrics.precision_score(
            y_true, y_pred, average="micro"
        ),
        "{}macro_precision".format(prefix): sklearn.metrics.precision_score(
            y_true, y_pred, average="macro"
        ),
        "{}micro_recall".format(prefix): sklearn.metrics.recall_score(
            y_true, y_pred, average="micro"
        ),
        "{}macro_recall".format(prefix): sklearn.metrics.recall_score(
            y_true, y_pred, average="macro"
        ),
        "{}hand_and_till_m_score".format(prefix): calc_hand_and_till_m_score(
            y_true, y_score
        ),
        "{}provost_and_domingos_auc".format(prefix): calc_provost_and_domingos_auc(
            y_true, y_score
        ),
    }
...
def collect_binary_classification_metrics(
    ...
    if y_true.shape[0] != y_probas_pred.shape[0]:
        msg = (
            "[ml_utils.collect_binary_classification_metrics]: y_true "
    ...
    if y_probas_pred.shape[1] != 2:
        msg = (
            "[ml_utils.collect_binary_classification_metrics]: "
    ...
    if len(prefix) > 0:
        ret = {"{}{}".format(prefix, key): value for key, value in ret.items()}
...
def _calc_hand_and_till_a_value(
    y_true: np.ndarray, y_score: np.ndarray, i: int, j: int
) -> float:
...
def calc_hand_and_till_m_score(y_true: np.ndarray, y_score: np.ndarray) -> float:
    ...
    **N.B.** This function *can* handle unobserved labels, except for the label
    ...
    if y_true.shape[0] != y_score.shape[0]:
        msg = (
            "[ml_utils.m_score]: y_true and y_score do not have matching "
    ...
    if y_score.shape[1] != (num_classes):
        msg = (
            "[ml_utils.m_score]: y_score does not have the expected "
