...
class ExternalSparseMatrixList:
    ...
    def __getitem__(self, key):
        ...
        if (self.sparse_matrix_list[key] is None) and (
            self.tarfile_contents is not None
        ):
            fn = "{}.mtx".format(key)
    ...
    def write(self, filename, overwrite=False):
        ...
        if os.path.exists(filename) and not overwrite:
            raise OSError(
                "Attempting to overwrite existing file: '{}'".format(filename)
            )
    ...
    def sum(self):
        ...
        """
        s = sum(
            matrix.sum() for matrix in self.sparse_matrix_list if matrix is not None
        )
        return s
    ...
    def max_len(self, axis=1):
        ...
        l = list(
            matrix.shape[axis]
            for matrix in self.sparse_matrix_list
...
def lazy_read_external_sparse_matrix_list(filename):
    ...
    esml = ExternalSparseMatrixList(
        size, tarfile_contents=contents, tarfile_names=names
    )
    return esml
...
def to_sparse_matrix(list_of_sparse_row_vectors):
    ...
    for i, sparse_row_vector in enumerate(list_of_sparse_row_vectors):
        if sparse_row_vector.shape[0] != 1:
            msg = (
                "list_of_sparse_row_vectors[{}] did not have the correct shape. "
                "Expected something of the form (1,X); found: ({},{})".format(
                    i, sparse_row_vector.shape[0], sparse_row_vector.shape[1]
                )
            )
...
def main():
    ...
    print(
        "TEST SUCCEEDED: The created sparse matrix list and the one read "
        "from disk are equal."
    )
