...
try:
    ...
    msg = (
        "Please install pytorch before using torch_utils. pyllars does not "
...
def send_data_to_device(
    *data: Sequence[torch.TensorType], device: torch.device
) -> Sequence[torch.TensorType]:
    ...
    *data : typing.Sequence[torch.TensorType]
    ...
    data = [_to_device(d, device) for d in data]
...
def retrieve_data_from_device(
    ...
    *data : typing.Sequence[torch.TensorType]
    ...
    data = [_from_device(d) for d in data]
...
def tensor_list_to_numpy(tensor_list: Sequence[torch.TensorType]) -> np.ndarray:
    ...
    """
    array = np.array([t.numpy() for t in tensor_list])
