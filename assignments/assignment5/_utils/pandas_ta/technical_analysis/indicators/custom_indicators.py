...
def ta_mamentum(df: pd.Series, period, real="Close", mas=None):
    ...
    @foreach_column
    @rename_with_parameters(
        function_name="mamemntum",
        parameter_names=["period", "mas"],
        output_names=["momentum", "overdrive", "drag"],
    )
        ...
        return ["momentum", "overdrive", "drag"], pd.concat(
            [
                ...
                rolling_apply(
                    mmeans,
                    2,
                    lambda x: np.sum(x.iloc[-1, 0].value >= x.iloc[0, 0].value) / scale
                    - 1,
                ),
                mmeans.apply(
                    lambda x: x[0].value[-1] / x[0].value[min(2, mas)] - 1, axis=1
                ),
                mmeans.apply(
                    lambda x: np.abs(x[0].value[0] - x[0].value[1:]).argmin(), axis=1
                ),
            ],
            axis=1,
            join="outer",
