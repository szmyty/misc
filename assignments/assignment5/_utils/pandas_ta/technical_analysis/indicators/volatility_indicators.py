...
@foreach_top_level_column
@rename_with_parameters(
    function_name="hurst_volatility",
    parameter_names=["period"],
    output_names=["H", "nu"],
)
def ta_hurst_volatility(
    df: pd.DataFrame,
    period=255 * 2,
    lags=30,
    open="Open",
    ...
    x = np.arange(1, lags)
    v = ta_gkyz_volatility(
        df, period=1, open=open, high=high, low=low, close=close
    ).iloc[:, 0]
    ...
    def hurst(sig):
        ...
        return np.array([model[0] / 2.0, np.sqrt(np.exp(model[1]))])

    e_hurst = np.array(
        [hurst(v.iloc[i - period + 1 : i]) for i in range(period - 1, len(df))]
    )
...
@foreach_top_level_column
@rename_with_parameters(
    function_name="gkyz_volatility",
    parameter_names=["period"],
    output_names=["gkyz_volatility"],
)
def ta_gkyz_volatility(
    df: pd.DataFrame, period=12, open="Open", high="High", low="Low", close="Close"
) -> pd.DataFrame:
...
@foreach_top_level_row
@foreach_top_level_column
def ta_cc_volatility(
    df: pd.Series | pd.DataFrame, period=12, real="Close"
) -> pd.DataFrame:
    data = get_columns(df, [real]) if real is not None and df.ndim > 1 else df

    @foreach_column
    @rename_with_parameters(
        function_name="cc_volatility",
        parameter_names=["period"],
        output_names=["cc_volatility"],
    )
    def wrapped_cc_volatility(col, period):
        ...
        return (
            columns,
            np.sqrt((np.log(col / col.shift(1)) ** 2).rolling(period).mean()).values,
        )
...
def ta_zscore(df: pd.Series | pd.DataFrame, period=12, real="Close") -> pd.DataFrame:
    ...
    @foreach_column
    @rename_with_parameters(
        function_name="zscore", parameter_names=["period"], output_names=["std", "z"]
    )
