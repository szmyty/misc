...
def create_wrapper_source_code():
    ...
    bar_indicators = {
        ...
        if desc["target_module"] != "price" and desc["type"] != "math"
    }
    write_source_file(
        os.path.join(
            ...
            for name, desc in indicators.items():
                ...
                f.write(
                    template.substitute(
                        ...
                        parameter_names=f"""'{"', '".join(desc['options'])}'"""
                        if len(desc["options"]) > 0
                        else "",
                        output_names=f"""'{"', '".join(desc['outputs'])}'"""
...
def discover_indicators():
    all_indicators = {
        name: get_info(func)
        for name, func in ti.__dict__.items()
        if hasattr(func, "full_name")
    }
    indicators = {
        name: desc for name, desc in all_indicators.items() if desc["type"] != "simple"
    }
    return indicators
