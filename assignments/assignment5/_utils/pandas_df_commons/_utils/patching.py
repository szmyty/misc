...
def _monkey_patch_dataframe(
    extension_field_name=None, extension_default_value=None, extension_class=None
):
    ...
    if existing is not None:
        if not isinstance(existing, property) or not isinstance(
            existing.fget(None), extension_class
        ):
            raise ValueError(f"field already exists as {type(existing)}")

    setattr(
        PandasObject, extension_field_name, property(lambda self: extension_class(self))
    )
