...
def _extender(df):
    ...
    from pandas_df_commons.extensions.functions import (
        cumapply,
        cumpct_change,
        rolling_apply,
        rescale,
...
def monkey_patch_dataframe(extender="X"):
    ...
    if existing is not None:
        if not isinstance(existing, property) or not str(
            existing.fget(None)
        ).startswith("<pandas_df_commons._extender.<locals>.Extender object"):
