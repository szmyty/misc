...
from pandas_df_commons._utils.streaming import frames_at_common_index, window
...
def rescale(df, range: Tuple[float, float], clip=False, axis=None):
    if axis is None:
        if df.ndim > 1:
            ...
            return pd.Series(
                ReScaler((df.min(), df.max()), range, clip)(df),
                name=df.name,
                index=df.index,
            )
    else:
        return df.apply(
            lambda x: ReScaler((x.min(), x.max()), range, clip)(x),
            axis=axis,
...
def sateful_apply(
    df,
    func: Callable[[Any, pd.DataFrame], Tuple[Any, pd.DataFrame]],
    start_state=None,
    **kwargs,
):
    state = [start_state]
...
def joint_apply(
    *df: pd.DataFrame,
    func: Callable[[Tuple[pd.DataFrame]], pd.Series],
    level=None,
    parallel=False,
):
    if parallel:
