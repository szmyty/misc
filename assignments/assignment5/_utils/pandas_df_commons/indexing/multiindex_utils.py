...
def unique_level_values(df: pd.DataFrame | pd.Index, level=0, axis=0):
    ...
    return (
        unique(idx.get_level_values(level)) if isinstance(idx, pd.MultiIndex) else idx
    )
...
def add_to_multi_index(df, head, inplace=False, axis=1, level=0):
    ...
    if axis == 0:
        df.index = pd.MultiIndex.from_tuples(
            [(head, *(t if isinstance(t, tuple) else (t,))) for t in df.index.tolist()]
        ).swaplevel(0, level)
    elif axis == 1:
        if df.ndim > 1:
            df.columns = pd.MultiIndex.from_tuples(
                [
                    (head, *(t if isinstance(t, tuple) else (t,)))
