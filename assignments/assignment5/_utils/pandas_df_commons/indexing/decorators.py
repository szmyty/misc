...
from typing import Any, Callable, Dict, T

import numpy as np
import pandas as pd
from pandas_df_commons._utils.multiprocessing import (
    blocking_parallel,
    streaming_parallel,
)
from pandas_df_commons.indexing._utils import (
    col_agg,
    get_top_level_columns,
    get_top_level_rows,
    loc_with_name,
    row_agg,
...
def foreach_column(func):
    ...
    def exec_on_each_column(df: pd.DataFrame, *args, **kwargs):
        if df.ndim > 1 and df.shape[1] > 0:
            results = [
                func(df[col], *args, **kwargs) for col in df.columns
            ]  # theoretically could be done parallel
            if results[0].ndim > 1 and results[0].shape[1] > 1:
                for i, col in enumerate(df.columns):
                    results[i].columns = pd.MultiIndex.from_product(
                        [[col], results[i].columns.tolist()]
                    )
...
def foreach_top_level(
    ...
    def decorator(func):
        ...
        def wrapper(df, *args, **kwargs):
            top_level_rows = (
                get_top_level_rows(df) if row_aggregator is not None else None
            )
            top_level_columns = (
                get_top_level_columns(df, level=col_level)
                if column_aggregator is not None
                else None
            )
            index_generator = top_level_separator_generator(
                df, top_level_rows, top_level_columns, col_level
            )
            ...
            if top_level_rows is None and top_level_columns is None:
                ...
                if parallel and parts > 1:
                    ...
                    results = streaming_parallel(
                        lambda indexes_sub_df: (
                            indexes_sub_df[0],
                            func(indexes_sub_df[1], *args, **kwargs),
                        ),
                        lambda: index_generator,
                    )
                else:
                    results = [
                        (idx, func(sub_df, *args, **kwargs))
                        for idx, sub_df in index_generator
                    ]
...
def foreach_top_level_row_and_column(
    parallel=False,
    row_aggregator=row_agg,
    column_aggregator=col_agg,
    progress_bar=False,
...
def foreach_top_level_column_aggregate(
    aggregator: Callable[[Dict[Any, T], int], T] = col_agg, level=0, parallel=False
):
...
def foreach_top_level_row_aggregate(
    aggregator: Callable[[Dict[Any, T]], T] = row_agg, parallel=False
):
