...
def intersection_of_index(*dfs: pd.DataFrame | pd.Index, level=None):
    intersect_index = (
        dfs[0] if level is not None and isinstance(dfs[0], pd.Index) else dfs[0].index
    )
    ...
    for i in range(1, len(dfs)):
        if dfs[i] is not None:
            other_index = (
                dfs[i]
                if level is not None and isinstance(dfs[i], pd.Index)
                else dfs[i].index
            )
...
def aligend_frames(*dfs: pd.DataFrame, level=None, dropna=False):
    idx = intersection_of_index(
        *[df.dropna() if dropna else df for df in dfs if df is not None], level=level
    )
