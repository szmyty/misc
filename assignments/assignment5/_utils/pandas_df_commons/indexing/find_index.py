...
def get_columns(
    frame: pd.DataFrame | pd.Series, keys
                ...
                # could be matching one level of a multiindex
                    ...
                    for column_tuple in frame.columns.tolist():
                        ...
                        # could be regex ~/<regex>/
                            ...
                            col.startswith("~") and col[1] == col[-1] and len(col) > 3
                        ):
                            column_matcher = "/".join(
                                [str(c).strip('"') for c in column_tuple]
                            )
                            cregex = col[2:-1]
    ...
    df = pd.DataFrame(
        ...
        if isinstance(frame.columns, pd.MultiIndex) and len(names) > 1
        else names,
        columns=frame.index,
    ).T
