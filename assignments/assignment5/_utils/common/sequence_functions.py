...
def lcs(a, b):
    ...
    for i, x in enumerate(a):
        for j, y in enumerate(b):
            tbl[i + 1][j + 1] = (
                tbl[i][j] + 1 if str(x) == str(y) else max(tbl[i + 1][j], tbl[i][j + 1])
            )
    res = []
    i, j = len(a), len(b)
    while i and j:
...
def find_sublist_indexes(complete_list, sub_list):
    ...
    mid_range_index_list = [
        x
        for x in complete_list_index[record_index_list[0] : record_index_list[-1] + 1]
...
def recursive_gen_nodecodes(dictionary, parrent_node, record_list, level, pre_counter):
    ...
    if type(dictionary) == dict:
        for k in dictionary.keys():
            ...
            recursive_gen_nodecodes(
                dictionary[k], k, record_list, level + 1, pre_counter
            )
    else:
