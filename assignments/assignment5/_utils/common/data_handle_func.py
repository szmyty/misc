...
from .excel_functions import refresh_excel_calculations, save_csv, write_format_columns
...
def check_mapping_complete(
    df_worksheet, complete_header_df, original2cn_dict, file_tag
):
    # 检查是否遗漏了不是“无”的映射字段`
    ...
    check_required_columns = [
        x
        for x in check_required_columns
        if "fillbeforeconcat:" not in x.replace(" ", "")
        or "合并前填充:" not in x.replace(" ", "")
    ]

    if check_required_columns:
        warning_msg = (
            "Warning: Failed to find the the mapping for column "
...
def dtype_handle(complete_header_df, dtype_dict, output=False):
    if dtype_dict:
        ...
        for dtype_column, dtype_type in dtype_dict.items():
            ...
            if (
                dtype_type_temp == ""
                or dtype_type_temp == "默认"
                or dtype_type_temp == "default"
            ):
                pass
                ...
                if "date" in dtype_type_temp:  # 如果是只要日期，把它转成没有时分秒的格式
                    complete_header_df[dtype_column] = complete_header_df[
                        dtype_column
                    ].dt.normalize()
                ...
                if (
                    output
                    and dtype_type_temp == "str"
                    and np.issubdtype(
                        complete_header_df[dtype_column].dtype, np.datetime64
                    )
                ):
                    complete_header_df[dtype_column] = complete_header_df[
                        dtype_column
                    ...
                    try:  # 防止有空值concat后全是float转str 其他非空的int也会带个小数点
                        complete_header_df[dtype_column] = (
                            complete_header_df[dtype_column]
                            .fillna("")
                            .astype(dtype_type)
                            .apply(
                                lambda x: x.rstrip(".0")
                                if type(x) == str and len(x) >= 2 and x[-2:] == ".0"
                                else x
                            )
                        )
                    except:
                        enter_exit(
                            f'Failed to convert column "{dtype_column}" to dtype "{dtype_type}"'
                        )
    ...
    if multi_plus_list:
        for m in multi_plus_list:
            ...
            for m_s in m_split_list[1:]:
                df_worksheet[m] = (
                    df_worksheet[m].fillna("").astype(str).replace("/", "")
                    + " "
...
def get_filter_condition_standardize_tag(filter_condition):
    ...
    if is_df_column(filter_condition):
        ...
        if len(filter_condition_list) < 2:
            enter_exit(
                f"Standardization:failed to compile filter condition:{filter_condition}!"
            )
        else:
...
def process_sort_order(join_table_df, join_columns, string):
    if string != "":
        ...
        reverse_ordere_new = [
            True for x in range(len(join_columns))
        ] + reverse_ordere_new

        join_table_df = join_table_df.sort_values(
            by=order_columns_new, ascending=reverse_ordere_new
        )
...
def calc_similarity(not_standard_str, new_standard_str, calc_func):
    ...
    match_list_no_space = calc_func(
        not_standard_str_no_space, new_standard_str_no_space
    )
    ...
    try:  # 也门 -- 对应被拆开的 卡塔尔 也门 科威特 阿曼,第一个卡塔尔拆出来之后 分母为0
        match_list_len_no_space_divide = len(not_standard_str_no_space) / len(
            new_standard_str_no_space
        )
    ...
    str_similarity = (
        match_list_len
        + continues_match
        + match_list_len_no_space
        + match_list_len_no_space_divide
    )
...
@catch_and_print
def check_similarity_number(
    not_standard_str, standard_dict, special_syn_list, ignore_punctuation=True
):
    ...
    for standard_str, target_str in standard_dict.items():
        ...
        if special_syn_list:
            syn_checking = check_syn_str_regex_number(
                not_standard_str, new_standard_str, special_syn_list
            )
...
def check_similarity_simple(
    ...
    for standard_str, target_str in standard_dict.items():
        ...
        if special_syn_list:
            syn_checking = check_syn_str_regex(
                not_standard_str, new_standard_str, special_syn_list
            )
    ...
    standard_similarity_list = sorted(
        standard_similarity_list, key=lambda x: x[0], reverse=True
    )
...
def check_similarity_strict(
    ...
    for standard_str, target_str in standard_dict.items():
        ...
        if str_similarity >= 2:
            ...
            first2_letter_equal = (
                not_standard_str[:2] == new_standard_str[:3].split(" ")[0][:2]
            )
            first3_letter_equal = (
                not_standard_str[:3] == new_standard_str[:3].split(" ")[0][:3]
            )

            # 前3位字符串包含2个数字的情况 必须满足前3个字母相同
            first3_letter_2num_not_standard = (
                re.search("[0-9]{2,}", not_standard_str[:3]) != None
            )
            first3_letter_2num_standard = (
                re.search("[0-9]{2,}", new_standard_str[:3]) != None
            )
            ...
            if len(not_standard_str) == 2:
                ...
                if not_standard_str == new_standard_str:
                    standard_similarity_list.append(
                        [str_similarity, standard_str, target_str]
                    )
                else:
                    standard_similarity_list.append(not_match_default)

            elif (
                len(not_standard_str) >= 3 and first2_letter_equal
            ) and (  # 输入字符 前3位有2个数字 + 标准字符 前3位也有2个数字 -- > 前3个必须要相等
                (
                    first3_letter_2num_not_standard
                    and first3_letter_2num_standard
                    and first3_letter_equal
                )  # 输入字符 前3位没有2个数字 + 标准字符 前3位也没有2个数字 -- > 前2个字符必须要相等
                or (
                    first3_letter_less2num_not_standard
                    and first3_letter_less2num_standard
                    and first2_letter_equal
                )
            ):
                standard_similarity_list.append(
                    [str_similarity, standard_str, target_str]
...
def standardize_by_similarity(
    ...
    standard_dict = dict(
        sorted(standard_dict.items(), key=lambda x: len(x[0]), reverse=False)
    )

    if mode == "simple_lcs":
        standard_similarity_list = check_similarity_simple(
            not_standard_str, standard_dict, special_syn_list, ignore_punctuation
        )
    elif mode == "number_similarity":
        standard_similarity_list = check_similarity_number(
            not_standard_str, standard_dict, special_syn_list, ignore_punctuation
        )
    else:
        standard_similarity_list = check_similarity_strict(
            not_standard_str, standard_dict, special_syn_list, ignore_punctuation
        )
...
def standardize_column_func(
    ...
    standard_similarity_list = standardize_by_similarity(
        not_standard_str,
        standard_dict,
        special_syn_list,
        replace_dict,
...
def read_csv_data(table_path):
    ...
    for encoding in ["utf-8", "gbk", "gb2312"]:
        try:
            ...
            print("")
            print(
                f"***** Failed to read file:{table_path} with encoding: {encoding} ***** "
            )
            print("")
            continue
...
def get_partial_not_match(
    ...
    partial_match_not_match_df = complete_header_df_notna.loc[
        (complete_header_df_notna[target_column_edit].isna() == True)
        | (complete_header_df_notna[target_column_edit] == ""),
        temp_columns,
    ]
...
def process_duplicates(df, drop_duplicates_condition):
    # 字段去重
    if (
        type(drop_duplicates_condition) == str
        and drop_duplicates_condition.strip() != ""
    ):
...
def replace_value_func(
    ...
    if replace_value_str != "":
        ...
        for replace_value in replace_values:
            ...
            if (
                replace_original != "" and "regex(" != replace_original[:6].lower()
            ):  # 支持regex替换
                complete_header_df_new[output_column] = complete_header_df_new[
                    output_column
                ...
                complete_header_df_new[output_column] = (
                    complete_header_df_new[output_column]
                    .apply(lambda x: re.sub(replace_original, "", x, flags=re.I))
                    .copy()
                )
            else:  # 如果原始内容为空，即特别需要填充空值,且不能用series.str
                complete_header_df_new[output_column] = (
                    complete_header_df_new[output_column]
                    .fillna(value=replace_result)
                    .replace("", replace_result, regex=False)
                )
...
@catch_and_print
def fillna_value_func(
    complete_header_df, complete_header_df_new, fillna_value_str, output_column
):
    if fillna_value_str != "":
        # 如果不是用别的字段做填充
        if not is_df_column(fillna_value_str):
            complete_header_df_new[output_column] = (
                complete_header_df_new[output_column]
                .fillna(value=fillna_value_str)
                .replace("", fillna_value_str)
            )
        # 如果是属于表格的字段作为填充
        else:
            ...
            if fillna_value_str_strip in complete_header_df_new.columns:
                complete_header_df_new[output_column] = (
                    ...
                    .replace("", float("nan"))
                    .fillna(value=complete_header_df_new[fillna_value_str_strip])
                )
            else:
                try:  # 想填充的可能是来自旧complete_header_df的字段
                    complete_header_df_new[fillna_value_str_strip] = complete_header_df[
                        fillna_value_str_strip
                    ]

                    complete_header_df_new[output_column] = (
                        complete_header_df_new[output_column]
                        .replace("", float("nan"))
                        .fillna(complete_header_df_new[fillna_value_str_strip])
                    )

                except KeyError:
                    enter_exit(
                        f"Fillna error: Column '{fillna_value_str_strip}' doesn't exist!"
                    )

                complete_header_df_new = complete_header_df_new.drop(
                    fillna_value_str_strip, axis=1
                )
...
def sort_value_func(complete_header_df_new, sort_column_list, sort_column_order_list):
    ...
    try:
        complete_header_df_new = complete_header_df_new.sort_values(
            by=sort_column_list, ascending=ascending_list
        )
    except:
...
def check_mapping_duplicates(df, target_cn_columns, table_stem=""):
    ...
    if count_duplicate and count_duplicate[0][1] > 1:
        enter_exit(
            'Table "{}" has {} original fields can be mapped to target field "{}" that leads to duplication!'.format(
                table_stem, count_duplicate[0][1], count_duplicate[0][0]
            )
        )
...
def get_save_sheet_name(config_table_name):
    sheet_name = re.sub(
        pattern="(.*)(\.xlsx|\.csv|.xls)", repl="\g<1>", string=config_table_name
    )
...
def save_results(
    ...
    if type(output_file_type) == str and output_file_type.lower() == "csv":
        ...
        print(
            "The total number of rows exceeds 1 million, result will be saved in CSV format."
        )
        save_csv(result_df[0], f"{save_name}.csv")

    else:
        write_format_columns(
            f"{save_name}.xlsx", result_df_list, sheet_name_list, min_column_width=14
        )
    ...
    if checking_result_list:
        ...
        write_format_columns(
            os.path.join(".\\result_checking", f"Checking results-{path_stem}.xlsx"),
            checking_result_list,
            checking_result_sheet_names,
            min_column_width=16,
        )
...
def process_join_table(
    ...
    join_table_df = join_table_df.apply(
        lambda x: x.fillna("").astype(str).str.strip().replace("", float("NaN"))
        if x.name in join_columns
        else x
    )
...
def process_match_complete_table(
    complete_header_df, source_columns, target_columns, join_columns, join_table_name
):
    # 转成str
...
def check_only_one_match_column(join_table_df, join_columns, target_columns):
    ...
    if (
        ...
        and join_columns[0] == target_columns[0]
    ):
