...
class Handler(object):
    ...
    def get_original2cn_dict(self, header_table_df, file_tag):
        ...
        if file_tag.lower() not in [
            x.lower() for x in header_table_df.columns.get_level_values(0)
        ]:
            file_tag = "Without file tag"

        header_column_index = (
            header_table_df.columns.get_level_values(0) == file_tag.lower()
        )

        header_table_df_c = header_table_df.iloc[:, header_column_index]

        header_table_first_three_c = header_table_df.loc[
            :, header_table_df.columns.get_level_values(0)[0]
        ]

        # 同时获取填充的
        for row, last_three in zip(
            header_table_df_c.iterrows(), header_table_first_three_c.iterrows()
        ):
            # 表头统一小写，换行符,空格全部去掉
            ...
            c_list = [
                x
                for x in a_list
                if split_colon(x)[0].lower().strip() == "fillbeforeconcat"
                or split_colon(x)[0].strip() == "合并前填充"
            ]
            ...
            if (
                b_list[1] != "默认" and b_list[1].lower() != "default" and b_list[1] != ""
            ) and b_list[2] != "":
    ...
    def concat_data(self):
        ...
        info_path_list = [
            x
            for x in info_path_list
            if "~$" not in x
            and (x[-5:].lower() == ".xlsx" or x[-4:].lower() in [".xls", ".csv"])
        ]
        ...
        for table_path in info_path_list:
            ...
            original2cn_dict, fillna_dict, dtype_dict = self.get_original2cn_dict(
                header_table_df, file_tag
            )

            if not original2cn_dict:
                enter_exit(
                    '"Data_processing_configuration" required mapping field "{}" not found !'.format(
                        file_tag
                    )
                )
            ...
            for sheets_property in sheets_property_list:
                ...
                if sheet_visibility == 0:  # 只读取可见的Sheet
                    ...
                    df_worksheet.columns = [
                        ...
                        for x in df_worksheet.columns
                    ]

                    df_worksheet = dropping_not_mapping(
                        df_worksheet, original2cn_dict, target_cn_columns
                    )
                    ...
                    df_work_sheet = drop_duplicated_columns_before_rename(
                        df_worksheet, original2cn_dict
                    )

                    df_worksheet = df_worksheet.rename(original2cn_dict, axis=1)

                    # 还必须要确认映射的字段没有重复，否则会影响到后面的数据列, 返回一个没有重复的字段列
                    df_work_sheet = check_mapping_duplicates(
                        df_worksheet, target_cn_columns, table_stem=table_stem
                    )
                    ...
                    if not df_worksheet.empty:
                        ...
                        complete_header_df = dtype_handle(
                            complete_header_df, dtype_dict
                        )
                        # 记录成功表格
                        success_sheet_df_list.append(
                            [table_stem, sheet, df_worksheet.shape[0]]
                        )
                        # complete_header_df 是一个完整的表头，可以避免concat造成的表头混乱/缺失，
                        # 但合并会导致字段全变成object(CSV文档，xlsx的输入不受影响)
                        complete_header_df = pd.concat(
                            [complete_header_df, df_worksheet],
                            axis=0,
                            sort=False,
                            ignore_index=True,
                        )
                        ...
                        print(
                            f'Getting from: "{table_stem}",Sheet:{sheet}, {df_worksheet.shape[0]} rows'
                        )

            success_sheet_df = pd.DataFrame(
                success_sheet_df_list,
                columns=["File Name", "Source Sheet", "Success reading records"],
            )
    ...
    def standardize_columns(self, complete_header_df, standardize_config_df):
        ...
        for row in table_values:
            ...
            # 先把空值的数据剔除再做模糊匹配
            complete_header_df_notna = complete_header_df.loc[
                complete_header_df[source_column].isna() == False, :
            ]
            complete_header_df_nan = complete_header_df.loc[
                complete_header_df[source_column].isna(), :
            ]
            # 过滤后全都是空值的话 直接继续下一行
            ...
            if standard_table_name != "" and standard_column.strip() != "":
                ...
                print(
                    f'-- Referencing from table "{standard_table_name}" column "{target_column}",Mode:{standardize_mode}'
                )
                standard_table_path = get_require_files(
                    self.require_file_dir, [standard_table_name]
                )[standard_table_name]
                # 统一将原始关联表转成str格式
                ...
                if order_columns != "":
                    standard_table_df = process_sort_order(
                        standard_table_df, [standard_column], order_columns
                    )

                # 需要对标准表做重复删除，类似字段匹配, 但不相同
                # 标准化前的第一层简单过滤
                (
                    filter_condition_2_columns_tag,
                    filter_left_column,
                    filter_right_column,
                ) = (False, "", "")
                ...
                if filter_condition != "" and filter_condition_2_columns_tag == False:
                    try:
                        ...
                        enter_exit(
                            f"Standardization: Failed to compile condition: {filter_condition}"
                        )

                # 全部转成半角符号
                standard_table_df[standard_column] = standard_table_df[
                    standard_column
                ].apply(lambda x: normalize_punctuations(x) if type(x) == str else x)
                complete_header_df_notna[source_column] = complete_header_df_notna[
                    source_column
                ].apply(lambda x: normalize_punctuations(x) if type(x) == str else x)
                ...
                if filter_condition_2_columns_tag:
                    ...
                    find_lack_columns(
                        standard_table_df,
                        [standard_column, target_column, filter_left_column],
                    )
                    # 需要保留一个模糊匹配需要获取的 target_column
                    standard_table_df_x = standard_table_df.loc[
                        :, [standard_column, target_column, filter_left_column]
                    ].drop_duplicates(
                        subset=[standard_column, filter_left_column], keep="first"
                    )

                    # 循环获取模糊匹配结果
                    if filter_right_column not in complete_header_df_notna.columns:
                        enter_exit(
                            f'Standardization Error: Cannot find column:"{filter_right_column}"'
                        )
                    # 记录每个的情况
                    complete_header_df_sub_list = []
                    for u in complete_header_df_notna[filter_right_column].unique():
                        temp_standard_df = standard_table_df_x.loc[
                            standard_table_df_x[filter_left_column] == u,
                            [standard_column, target_column],
                        ]
                        ...
                        standard_dict = {
                            x: y
                            for x, y in zip(
                                standard_dict[standard_column].values(),
                        ...
                        complete_header_df_sub[temp_column] = (
                            complete_header_df_sub[source_column]
                            .fillna(value="")
                            .astype(str)
                        )

                        complete_header_df_sub[temp_column] = complete_header_df_sub[
                            temp_column
                        ].apply(
                    ...
                    if len(complete_header_df_sub_list) == 1:
                        ...
                        complete_header_df_notna = pd.concat(
                            complete_header_df_sub_list, axis=0, ignore_index=True
                        )
                ...
                # 如果是普通的df quiry过滤方式
                    ...
                    lack_column_list = find_lack_columns(
                        standard_table_df,
                        [standard_column, target_column],
                        "Standardization reference table",
                    )
                    # 提取出标准列表
                    standard_dict = standard_table_df.loc[
                        :, [standard_column, target_column]
                    ].to_dict()
                    standard_dict = {
                        x: y
                    ...
                    complete_header_df_notna[temp_column] = complete_header_df_notna[
                        source_column
                    ].apply(
                        lambda x: standardize_column_func(
                            x,
                ...
                # 空的和处理过的非空数据记得合并
                complete_header_df = pd.concat(
                    [complete_header_df_notna, complete_header_df_nan],
                    axis=0,
                    ignore_index=True,
                )

                # 需要记录这两个字段分别有哪些记录匹配不上
                partial_match_not_match_df = get_partial_not_match(
                    complete_header_df_notna,
                    row_counter,
                    source_column,
                    standard_table_name,
                    standard_column,
        ...
        if partial_match_not_match_df_list:
            partial_match_not_match_df = pd.concat(
                partial_match_not_match_df_list, axis=0, ignore_index=True
            )
    ...
    def split_columns(self, complete_header_df, split_table_df):
        ...
        for row in split_table_values:
            ...
            if (
                split_table_column != ""
                and split_table_name.strip() != ""
                and split_column.strip() != ""
            ):
                split_file_dict = get_require_files(
                    self.require_file_dir, [split_table_name]
                )
                split_file_path = split_file_dict[split_table_name]
                split_standard_df = read_config_table(split_file_path)
                split_standard_df.columns = [
                    x.strip() if type(x) == str else x
                    for x in split_standard_df.columns
                ]
                ...
                if split_table_column not in split_standard_df.columns:
                    enter_exit(
                        f'Failed to find column "{split_table_column}" in table "{split_table_name}" '
                    )
                else:
                    split_standard_list = [
                        x
                        for x in split_standard_df[split_table_column].tolist()
                        if x == x and x != ""
                    ]
                    split_standard_list = sorted(
                        split_standard_list, key=len, reverse=True
                    )
                split_config_dict.update(
                    {split_column: [split_standard_list, split_symbol]}
                )
            else:
                print(
                    f'Standard column used to split is empty, "{split_symbol}" will be the symbol used to split'
                )
                split_config_dict.update({split_column: [[], split_symbol]})

        complete_header_df = split_column_by_words(
            complete_header_df, split_config_dict, mode=split_mode
        )
    ...
    def match_columns(self, complete_header_df, match_table_df):
        ...
        for row in table_values:
            ...
            if (
                source_columns != ""
                and len(join_columns) > 0
                and len(target_columns) > 0
            ):
                # 如果两边的字段数量不一致
                ...
                if join_table_name != "":
                    join_table_path = get_require_files(
                        self.require_file_dir, [join_table_name]
                    )[join_table_name]
                ...
                # 检查两张表 有没有字段缺失
                find_lack_columns(
                    complete_header_df, source_columns, "Matching, complete_header_df"
                )
                find_lack_columns(
                    join_table_df,
                    set(join_columns + target_columns),
                    "Matching, join_table",
                )

                join_table_df = process_join_table(
                    join_table_df=join_table_df,
                ...
                complete_header_df = process_match_complete_table(
                ...
                join_table_df, only_one_match_column = check_only_one_match_column(
                    join_table_df, join_columns, target_columns
                )

                # 匹配默认忽略大小写
                if (
                    type(match_mode) == str
                    and match_mode.lower().replace("-", "") == "case sensitive"
                ):
                ...
                if only_one_match_column:
                    complete_header_df = complete_header_df.rename(
                        {"additional_temp": target_columns_edit[0]}, axis=1
                    )
                ...
                not_match_df = complete_header_df.loc[
                    complete_header_df[target_columns_edit[0]].isna(), source_columns
                ].drop_duplicates()

                not_match_df["Content"] = not_match_df[source_columns].agg(
                    "+".join, axis=1
                )

                not_match_df["Matching field"] = (
                    "+".join(source_columns) + " Match " + "+".join(join_columns)
                )
        ...
        for row in table_values:
            ...
            if input_column != "" and output_column != "":
                ...
                if input_column not in seen_columns:
                    complete_header_df_new[output_column] = complete_header_df[
                        input_column
                    ].fillna("")
    ...
    def regex_extraction(self, complete_header_df, regex_extraction_table):
        ...
        for row in table_values:
            ...
            if source_column != "" and extract_pattten != "":
                ...
                complete_header_df[output_column] = complete_header_df[
                    source_column
                ].apply(
                    lambda x: "\n".join(
                        re.findall(extract_pattten, x, flags=re.I)
                    ).strip()
                    if type(x) == str
                    else ""
                )
    ...
    def calc_time(self, data_df, time_process_df):
        if not time_process_df.empty and not data_df.empty:
            for row in time_process_df.values:
                ...
                # 获取需要统计的时间区间
                re_result = re.search(
                    "^([a-zA-Z\u4e00-\u9fa5]{1,}).?(\d{1,})$", original_period
                )
                ...
                try:
                    ...
                    enter_exit(
                        "Time range input error: Make sure the input time range is like day_1, day_2, week_1, etc"
                    )
                ...
                if period in ["day", "days", "天", "日", "d"]:
                    data_df[original_period] = (
                        enddate - data_df[time_column]
                    ) / np.timedelta64(period_number, "D")
                elif period in ["week", "weeks", "周", "星期", "w"]:
                    data_df[original_period] = (
                        enddate - data_df[time_column]
                    ) / np.timedelta64(period_number, "W")
                elif period in ["month", "months", "月", "月数", "m"]:
                    data_df[original_period] = (
                        enddate - data_df[time_column]
                    ) / np.timedelta64(period_number, "M")
                elif period in ["year", "years", "年", "年数", "y"]:
                    data_df[original_period] = (
                        enddate - data_df[time_column]
                    ) / np.timedelta64(period_number, "Y")
                elif period in ["hour", "hours", "时", "小时", "h"]:
                    data_df[original_period] = (
                        enddate - data_df[time_column]
                    ) / np.timedelta64(period_number, "h")
                ...
                if not data_df_empty_date.empty:
                    enter_exit(
                        'Time process error: column "{}" has {} empty datetime rows after combining all the data files!'.format(
                            time_column, data_df_empty_date.shape[0]
                        )
                    )
                # 往上取整，统一转成int格式
                data_df[original_period] = (
                    data_df[original_period].apply(np.ceil).astype(int)
                )
    ...
    def process_statistic_groups(self, data_df, statistic_groups_df):
        ...
        statistic_groups_df[group_column] = (
            statistic_groups_df[group_column]
            .replace("", float("Nan"))
            .fillna(method="ffill")
        )
            ...
            if value_column:
                ...
                if "word" in agg_func:  # 如果为空 返回空字典
                    if data_df_sub.empty:
                        agg_df_sub = create_group_empty(
                            data_df, groups, value_column, fillna=r"{}"
                        )
                    else:
                        require_file_dict = get_require_files(
                            self.require_file_dir, ["keyword", "stop_word"]
                        )
                        keyword_dict, keyword_format_dict = get_keyword_dict(
                            require_file_dict["keyword"]
                        )
                        stopword_dict, stopword_format_dict = get_keyword_dict(
                            require_file_dict["stop_word"]
                        )

                        keyword_list = convert_key2list(keyword_dict)
                        stopword_list = convert_key2list(stopword_dict)
                        # 词频统计需要传入关键词字典
                        agg_df_sub = group_basic_agg(
                            data_df_sub,
                            groups,
                            agg_func,
                            value_column,
                    ...
                    if data_df_sub.empty:
                        agg_df_sub = create_group_empty(
                            data_df, groups, value_column, fillna=0
                        )
                    else:
                        agg_df_sub = group_basic_agg(
                            data_df_sub,
        ...
        for row in calculation_df.values:
            ...
            if calc_content != "":
                try:
                    ...
                    enter_exit(
                        f" Calculation Error on row {counter}, \n Content:{calc_content}"
                    )
        ...
        for row in pivot_table_df.values:
            ...
            if all([x == x and x != "" for x in row[:-1]]):
                ...
                data_df = pd.pivot_table(
                    data_df, index=index, columns=columns, values=values
                )
