...
def pd_save_excel(df, save_path, startrow=0, startcol=0):
    ...
    while close_tag <= 0:
        try:
            ...
            input(
                '\nFailed to write file!\n  Please Close "{}" Then Press Enter to Continue'.format(
                    save_path
                )
            )
...
def autofit_column_width(
    xlsxwriter_ws, header_columns, content_columns=[], wrap_text=False, **kwargs
):
    ...
    if wrap_text == False:
        length_list = [
            len("".join(re.findall(cn_pat, c))) * 2
            + len("".join(re.findall(en_pat, c))) * 1
            + 4
        ...
        length_list = [
            math.ceil(len("".join(re.findall(cn_pat, c))) / 2) * 2
            + math.ceil(len("".join(re.findall(en_pat, c))) / 2) * 1
            + 2
            for c in list(header_columns)
        ]
        ...
        # 确保每列的宽度至少是4
        length_list = [
            x if x >= min_column_width else min_column_width for x in length_list
        ]
...
def adjust_pct_decimal_format(original_number_format, data):
    ...
    biggest_decimal_places = max(
        [abs(Decimal(str(x)).as_tuple().exponent) for x in data if type(x) == float]
    )

    # 当约到某一位的数字超过50%时，加一个小数位
    data_decimal_check = [
        round(x, original_decimal_places + 2) for x in data if type(x) == float
    ]
    ...
    if (
        biggest_decimal_places > original_decimal_places
        and mode_counter / len(data_decimal_check) >= 0.5
    ):
...
def write_row_format(xlsxwriter_ws, data, start_pos, cell_format={}, direction=0):
    ...
    if direction == 1:
        xlsxwriter_ws.write_column(
            row=0, col=start_pos, data=data, cell_format=cell_format
        )
    else:
        xlsxwriter_ws.write_row(
            row=start_pos, col=0, data=data, cell_format=cell_format
        )
...
def write_multi_tables(xlsxwriter_wb, sheet_name, df_list, direction=0, **kwargs):
    ...
    normal_format = kwargs.get(
        "normal_format", {"font_name": "calibri", "font_size": 11}
    )
    ...
    header_format = xlsxwriter_wb.add_format(
        {
            "font_name": "微软雅黑",
            "bold": False,
            "font_size": 11,
    ...
    for df in df_list:
        ...
        for i, data in data_set:
            if is_header:
                ...
                # 如果是日期，转成对应的日期格式
                data = [
                    datetime.datetime.strftime(x, date_format)
                ...
                if pct_tag:
                    ...
                    pct_format = xlsxwriter_wb.add_format(
                        {
                            "font_name": "calibri",
                            "font_size": 11,
...
def write_pct_columns(
    save_path, df_list, sheet_name_list=[], pct_columns=[], content_columns=[], **kwargs
):
    ...
    date_format = xlsxwriter_wb.add_format(
        {"font_name": "calibri", "font_size": 11, "num_format": "yyyy/mm/dd"}
    )
    pct_format = xlsxwriter_wb.add_format(
        {"font_name": "calibri", "font_size": 11, "num_format": "0.00%"}
    )
    text_format = xlsxwriter_wb.add_format(
        {"font_name": "calibri", "font_size": 11, "num_format": "@"}
    )
    ...
    for df, sheet_name in zip(df_list, sheet_name_list):
        ...
        if not df.empty:
            pct_columns = [
                x for x in header_columns if [y for y in pct_columns if y in x]
            ]
            # 写入表头
            xlsxwriter_ws.write_row(
                row=0, col=0, data=header_columns, cell_format=header_format
            )
...
def write_format_columns(
    ...
    **kwargs,
    ...
    normal_format = xlsxwriter_wb.add_format({"font_name": "calibri", "font_size": 11})
    round_format = xlsxwriter_wb.add_format(
        {"font_name": "calibri", "font_size": 11, "num_format": "0.00"}
    )
    ...
    for df, sheet_name in zip(df_list, sheet_name_list):
        ...
        if not df.empty:
            ...
            for column_index, column in enumerate(header_columns):
                column_values = df[column].to_list()
                if (
                    column in pct_columns
                    or len([x for x in pct_list if x in column]) > 0
                ):  # 如果属于百分比列
                    column_format = pct_format
                elif (
                    column in round_columns
                    or len([x for x in round_list if x in column]) > 0
                ):  # 如果属于需要round的列
                ...
                # 	column_format = round_format
                    ...
                    column_values = [
                        x if x == x and type(x) == str else x for x in column_values
                    ]
                    column_values = [x if x == x else "" for x in column_values]
                    column_format = normal_format
