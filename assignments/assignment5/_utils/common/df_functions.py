...
from .os_functions import enter_exit, generate_md5, last_day_of_month
    ...
    for c in group_columns:
        ...
        try:  # 是否是数字列， 如果是获取最大最小值, 通常不会拿float当成索引
            ...
            if column_max < 5:  # 最小保证要是5以上(week_1 报错)
                ...
                column_max >= 5 and column_max <= 15
            ):  # 临时限定补充的部分不超过12，通常计算0不会超过这个(天/周维度)，否则计算的数据量太大
    ...
    if type(condition) == str and condition.strip() != "":
        try:
            ...
            write_format_columns(
                "Filter condition-Result when error.xlsx", df, "content"
            )
            enter_exit(
...
def column_gen_md5(df, unique_id_columns, target_column="MD5"):
    # 几列合并 sum(多个字段)
    df[target_column] = (
        df.loc[:, unique_id_columns]
        .fillna("")
        .astype(str)
        .sum(1)
        .apply(lambda x: generate_md5(x))
    )
    return df
...
def merge_case_insensitive(df_1, df_2, how, on):
    ...
    for i, o in enumerate(on):
        ...
        df_1[temp_name] = (
            ...
            .fillna("")
            .astype(str)
            .apply(lambda x: normalize_punctuations(x).lower().strip())
        )
        df_2[temp_name] = (
            df_2[o]
            .fillna("")
...
def process_enddate(df):
    ...
    df["enddate"] = df["enddate"].apply(
        lambda x: datetime.datetime(get_year_month(x)[0], get_year_month(x)[1], 1)
        if get_year_month(x) != None
...
def pivot2multi_tables(
    df, index, columns, value_list, filt_column=None, filt_target=None, fillna=None
):
...
def delete_unnamed_behind(df):
    ...
    unnamed_columns = [
        ...
        if (type(x) == str and re.search("unnamed", x, flags=re.I) != None)
    ]
...
def check_abnormal_dates(df_worksheet, date_column, table_path, sheet):
    ...
    if not check_date_empty.empty:
        ...
        print(
            "日期不允许为空：文档:{},来源Sheet:{},第{}行，请补充以上日期再运行".format(
                table_path, sheet, empty_date_row
            )
        )
        input("回车键退出")
        sys.exit()
    ...
    df_worksheet[date_column] = df_worksheet[date_column].apply(
        lambda x: datetime.strftime(xldate_as_datetime(x, 0), "%Y-%m-%d")
...
def normalize_dates(df_worksheet, date_columns, table_path="默认", sheet="默认"):
    ...
    for date_column in date_columns:
        ...
        if not check_date_empty.empty:
            ...
            empty_date_row = ",".join(empty_date_row)
            enter_exit(
                "Date must not be empty：File:{},Sheet:{},Row:{},Please add an valid date then try again!".format(
                    table_path, sheet, empty_date_row
                )
            )
...
def copy_seperate_header_columns(df, pattern):
    ...
    for c in level_0_columns:
        ...
        if len(match_list) > 1:
            for m in match_list:
                # 需要将多列分别赋值
                df_copy_need_copy = df_copy.loc[
                    :, df_copy.columns.get_level_values(0) == c
                ]
...
def split_column_by_words(df, split_words_dict, mode="split"):
    ...
    for column, split_standard_list in split_words_dict.items():
        print(
            f"Splitting column:{column},length of standard keywords:{len(split_standard_list[0])},mode:{mode}",
        )
        # 先获取到分隔符
        split_symbol = split_standard_list[1]
        # #全角转半角。去掉重复符号
        split_standard_list = [
            normalize_punctuations(x) for x in split_standard_list[0]
        ]
        ...
        split_standard_list = sorted(
            split_standard_list, key=lambda x: len(x), reverse=True
        )
        ...
        if split_standard_list:
            ...
            df[column] = (
                ...
                .fillna("")
                .astype(str)
                .apply(lambda x: normalize_punctuations(x))
            )
            ...
            if "split" in mode:
                df_column_notnull[column] = regex_split_column(
                    df_column_notnull[column], split_pat, split_symbol
                )
            elif "findall" in mode:
                df_column_notnull[column] = df_column_notnull[column].apply(
                    lambda x: re.findall(split_pat, x)
                )
            elif "fast" in mode or "flash" in mode:
                kw_processor = get_flashtext_processor(split_standard_list)
                df_column_notnull[column] = df_column_notnull[column].apply(
                    lambda x: kw_processor.extract_keywords(x)
                )
            ...
            # 重新合并
            df = pd.concat(
                [df_column_null, df_column_notnull], axis=0, ignore_index=True
            )
...
def stack_list_column(df, split_column):
    ...
    for index, value_list in zip(
        df_copy[split_column].index, df_copy[split_column].values
    ):
...
def expand_to_columns_pd(df):
    ...
    gc.collect()
    # 根据最多的列决定如何读取table
    new_columns = [unique_key] + [
        expand_column + " " + str(x) for x in range(1, maximum_ecc_column + 1)
    ]
    # 再把数据读回来
...
def stack_columns_to_multi_row(df, target_stack_name=None, regex_foramt=None):
    ...
    if regex_foramt == None:
        ...
        stack_columns = [
            x
            for x in df_columns
            if re.match(stack_column_pat, x) != None and re.match(regex_foramt) != None
        ]
    ...
    stack_number_list = [
        re.match(stack_column_pat, x).group()
        for x in df_columns
        if re.match(stack_column_pat, x) != None and re.match(regex_foramt) != None
    ]
    # 保存数据
    ...
    if len(stack_columns) < 2:
        ...
        for i, row in df.loc[:, stack_columns].iterrows():
            stack_line = ",".join(
                sorted([x for x in row.values if type(x) == str and x != "nan"])
            )
...
def get_target_sheet(path, target_name):
    ...
    if len(match_list) > 1:
        print(
            '"{0}"存在{1}份sheet文档,正在读取{2}'.format(
                target_name, len(match_list), match_sheetname
            )
        )
...
def group_by_concat(df, group_column, agg_func_column):
    # 防止改到另外的部分
    df_copy = df.copy()
    df_copy[agg_func_column] = (
        df_copy[agg_func_column]
        .fillna("")
        .apply(lambda x: x if type(x) == str and x != "" else str(x))
    )
...
def group_word_count(
    ...
    df_agg[agg_func_column] = (
        df_agg[agg_func_column]
        .swifter.progress_bar(enable=True, desc=agg_func)
        .apply(
            lambda x: get_word_freq_dict(
                x,
                keyword_list,
                stopword_list,
                count_keywords_only=count_keywords_only,
                word_num=200,
            )
        )
    )
...
def group_dict_sum(df, group_column, agg_func_column):
    df[agg_func_column] = df[agg_func_column].apply(
    ...
    # 转成json,文本格式
    df_agg[agg_func_column] = df_agg[agg_func_column].apply(
        lambda x: json.dumps(x, ensure_ascii=False) if isinstance(x, dict) else r"{}"
    )
...
def word_agg_func(
    df, group_column, agg_func, agg_func_column, keyword_list, stopword_list
):
    # 如果是统计的关键词，停用词直接置空
...
def group_basic_agg(
    ...
    lack_column_list = find_lack_columns(
        df_copy, agg_func_column + group_column, "Group statistic"
    )
    ...
    if "word" in agg_func:
        ...
        try:
            df_copy_agg = (
                ...
                .agg(agg_func)
                .sort_values(by=agg_func_column, ascending=False)
                .fillna(0)
            )
...
def get_2_columns_div(df, dividend, divisor):
    ...
    :param divisor :除数   分母
    """
    result_series = (df[dividend] * 100 / df[divisor]).apply(
        lambda x: "{0:0.2f}%".format(x) if type(x) == float else x
    )
...
def get_outer_join_diff(df_list, on=None, left_right=None):
    ...
    outer_join_diff_df = outer_join_df.loc[
        (outer_join_df["table_source_1"].isna() == True)
        | (outer_join_df["table_source_2"].isna() == True),
        :,
    ]
    # 填充构建一个新的合并来源列
    outer_join_diff_df["table_source_1"].fillna(
        value=outer_join_diff_df["table_source_2"], inplace=True
    )
