...
from .regex_functions import get_keyword_pat, replace_punctuations, replace_re_special
...
def process_text_eng(
    text, keyword_list=[], stopword_list=[], count_keywords_only=False
):
    # 仅适用英文
    ...
    text = (
        replace_punctuations(text, replace_to_symbol=" ", exclude=["@"]).lower().strip()
    )
...
def get_word_freq_dict(
    text, keyword_list, stopword_list, count_keywords_only=False, word_num=200
):
    text_list = process_text_eng(
        text, keyword_list, stopword_list, count_keywords_only=count_keywords_only
    )
