...
Please put the Keywords.xlsx to current folder and Press Enter to continue'
    ...
    if not header_list:
        print(
            "Keywords.xlsx should have table header/headers such as red,blue,bold etc."
        )
        input("\nPress Enter to exit")
        sys.exit()
    ...
    for row in list(ws.get_rows())[1:]:
        for i, format_word in enumerate(header_list):
            if format_word != None:
                ...
                if (
                    type(keyword_value) == float
                    and math.ceil(keyword_value) == keyword_value
                ):
...
def write2wb(
    xlsxwriter_wb, highlight_column_list, keyword_dict, highlight_column="Text"
):
    # 获取insert_dict
    ...
    for row_index, value in enumerate(highlight_column_list):  # [7743:7745]
        ...
        if type(params) == str:
            ...
            success = xlsxwriter_ws.write_rich_string(
                row_index + 1, column_index, *params
            )
            # 如果返回是-5代表列表只有两个元素，不能写入,需要把文本取出来重新写
            if success == -5:
                one_word_text = params[-1]
                success = xlsxwriter_ws.write(
                    row_index + 1, column_index, one_word_text, params[0]
                )
        else:
            xlsxwriter_ws.write(row_index + 1, column_index, params)
        ...
        if keyword_record_dict:
            ...
            for k, v in keyword_record_dict.items():
                ...
                v_text = ",".join(v)
                xlsxwriter_ws_target.write(
                    row_index + 1, column_index, v_text, cell_format
                )
...
def highlight(xlsxwriter_wb, xlrd_ws, highlight_column, keyword_dict):
    ...
    xlsxwriter_wb = write2wb(
        xlsxwriter_wb, highlight_column_series, keyword_dict, highlight_column
    )
