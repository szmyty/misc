...
def check_syn_str_regex(string_a, string_b, special_syn_list):
    ...
    match_result_a = [
        re.search(x, string_a, flags=re.I).group()
        for x in special_syn_list
        if re.search(x, string_a, flags=re.I) != None
    ]
    match_result_b = [
        re.search(x, string_b, flags=re.I).group()
        for x in special_syn_list
        if re.search(x, string_b, flags=re.I) != None
    ]
...
def check_syn_str_regex_number(string_a, string_b, special_syn_list):
    ...
    match_result_a = [
        True if re.search(x, string_a, flags=re.I) != None else False
        for x in special_syn_list
    ]
    match_result_b = [
        True if re.search(x, string_b, flags=re.I) != None else False
        for x in special_syn_list
    ]
...
def replace_punctuations(input_string, replace_to_symbol=" ", exclude=[]):
    ...
    mapping_result = "".join(
        [replace_to_symbol if s not in exclude else s for s in string.punctuation]
    )
...
def search_en(combine_string):
    ...
    if found_all:  # 定位有英文的部分
        en_found_all = [
            re.search(en_pat, x).group()
            for x in found_all
            if re.search(en_pat, x) != None
        ]
...
def partial_match_pct(short_str, long_str, special_syn=["pro", "plus", "max", "youth"]):
    ...
    # 至少出现3个字符,并且前两个字母(如果前两位是字母+数字，后面不再有数字),如果前3位包含2个数字,前3个字符要相同,规避掉V 15这种空格的情况
        ...
        or (
            first3_letter_less2num_short
            and first3_letter_less2num_long
        ...
        if " " in long_str or "(" in long_str:
            long_str_bias = (
                len(match_short_str) / len(long_str.split(" ")[0].split("(")[0]) / 100
            )  # 比例需要调小
        ...
        if check_syn_str(short_str, special_syn) == check_syn_str(
            new_long_str, special_syn
        ):
...
def re_sub_replace(pattern, string, repace_symbols=("-", "_"), group_name=1):
    ...
    def replace_match(matched):
        ...
        replace_string = original_string.replace(
            matched_string,
            matched_string.replace(replace_symbols[0], replace_symbols[1]),
        )
...
def re_findall_replace(pattern, string, replace_symbols=("-", "_")):
    ...
    for mat in matched_list:
        new_string = new_string.replace(
            mat, mat.replace(replace_symbols[0], replace_symbols[1])
        )
    return new_string
    ...
    if matched_list:
        for mat in matched_list:
            ...
            if match_sub != None:
                ...
                replace_matched = split_wrong_combine(
                    pattern, sub_pattern, replace_match
                )
