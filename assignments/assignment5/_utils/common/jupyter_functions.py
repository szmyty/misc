...
def nbconverter(notebookPath, directionPath=None):
    ...
    source = [
        re.sub(pattern=pattern_variables, repl=print_match, string=t) for t in source
    ]
    source = [
        re.sub(pattern=pattern_square_bracket, repl=print_match, string=t)
        for t in source
    ]
    source = [
        re.sub(pattern=pattern_string, repl=print_match, string=t) for t in source
    ]
...
nbconverter(
    r"C:\Users\Administrator.DG-11030335\Scripts\voc_alarm\外销意见反馈预警监控\convert2alarm.ipynb"
)
