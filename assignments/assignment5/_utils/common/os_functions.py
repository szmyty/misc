...
def get_folder_list(path=".\\", folder_name=""):
    if folder_name != "":
        folder_list = [
            ...
            and folder_name.lower().strip() in f.lower()
            and f != "__pycache__"
        ]
    else:
        folder_list = [
            f
            for f in os.listdir(path)
            if os.path.isfile(os.path.join(path, f)) == False and f != "__pycache__"
        ]
...
def choose_folder(path=r".\\", folder_name=""):
    ...
    if len(folder_list) == 0:
        ...
        folder_choose_index = input(
            "\n---Please Select Folder Number(Default:First Folder): "
        ).strip()
        # 打开文档
...
def check_require_files(path, require_name_list, regex=False):
    ...
    if regex == False:
        require_list = [
            x for x in require_name_list if "~$" not in x and x in path_files
        ]
    else:
        require_dict = defaultdict(str)
        require_list = [
            (x, re.search(x, y).group())
            for x in require_name_list
            for y in path_files
...
def get_require_files(
    path, require_file_list, regex=True, matched_part="xls", if_walk_path=True
):
    """
    检查某个路径是否包含必须的文档
    :param path:路径
    ...
    if regex == False:
        require_list = [
            (x, y)
            for x in require_file_list
            for y in path_files
            if "~$" not in x and ".py" not in x and x == y.split("\\")[-1]
        ]
    else:
        if matched_part == None:
            require_list = [
                ...
                for x in require_file_list
                for y in path_files
                if x.lower() in y.lower().split("\\")[-1] and "~$" not in y
            ]
        else:
            require_list = [
                (x, y)
                for x in require_file_list
                for y in path_files
                if x.lower() in y.lower().split("\\")[-1]
                and "~$" not in y
                and matched_part in y
            ]
...
def choose_file(path=r".\\"):
    ...
    if len(file_list) == 0:
        ...
        file_choose_index = input(
            "\n---Please Select File Number(Default:First File): "
        ).strip()
        # 打开文档
...
def choose_sheet_column(file_path):
    ...
    if len(sheet_names) == 1:
        ...
        sheet_choose_index = input(
            "\n---Please input Sheet Number and press Enter(Default:First sheet): "
        ).strip()
        # 打开文档
    ...
    if len(columns) == 1:
        ...
        column_choose_index = input(
            "\n---Please input the Column Number that needs highlighted（Default:main_body）:"
        ).strip()
        # 打开文档
