...
def get_connection_by_config(
    database_type="mysql", config_path="connection_config.ini"
):
...
def get_alter_index_text(conn, table_name):
    ...
    if add_index_list:
        add_index_text = text(
            "alter table {} ".format(table_name) + ",".join(add_index_list) + ";"
        ).execution_options(autocommit=True)
        del_index_text = text(
            "alter table {} ".format(table_name) + ",".join(del_index_list) + ";"
        ).execution_options(autocommit=True)
...
def write2table(engine_text, df, table_name, how="normal"):
    ...
    if how == "normal":
        df.to_sql(
            table_name, con=conn, if_exists="append", index=False, chunksize=100000
        )
    ...
    # 如果采用第二种方式写入
        ...
        if auto_increment_key != "":
            load_infile_sql = r"""  LOAD DATA INFILE '{0}'
                                    ...
                                    """.format(
                temp_path, table_name, ",".join(header_columns), auto_increment_key
            )
        else:
            load_infile_sql = r"""  LOAD DATA INFILE '{0}'
                        INTO TABLE {1} 
    ...
    try:
        ...
        if unique_columns:  # 如果存在唯一性约束（包括主键约束）
            ...
            for u in update_columns:
                sql_duplicate_update = """{0}.{1} = temp_insert.{1}""".format(
                    table_name, u
                )
            ...
            sql_insert_update = text(
                """insert ignore into {0} 
                                       ...
                                       {1};""".format(
                    table_name, sql_duplicate_update
                )
            )
...
if __name__ == "__main__":
    engine_text = gen_engine_text(
        r"E:\脚本\get_sql_data-master\connection_config.ini", "oracle+cx_oracle"
    )
