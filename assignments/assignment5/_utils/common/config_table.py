...
from .df_functions import (
    check_abnormal_dates,
    copy_seperate_header_columns,
    df_fillna_str,
    normalize_multi_header,
    stack_list_column,
)
...
class ConfigReader(object):
    ...
    def get_header_table(self, header_table_df: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    def get_complete_header_df(
        self, header_table_df: pd.DataFrame
        ...
        target_cn_columns = header_table_df[header_table_columns[2]][
            :target_column_num
        ].tolist()
    ...
    def get_config_tables(self, if_walk_path=True):
        require_file_dict = get_require_files(
            self.config_file_dir, self.config_table_name, if_walk_path=if_walk_path
        )
                    ...
                    if config in sheet.lower().strip():
                        ...
                        if "mapping" in config:
                            ...
                            (
                                complete_header_df,
                                target_cn_columns,
                            ) = self.get_complete_header_df(table)
                        else:
